	...
	player.events.on(GuildQueueEvent.PlayerStart, async (queue, track) => {
		...
		if (!queue.metadata || !queue.metadata.channel) {
			const defaultChannel = queue.guild.channels.cache.find(
				(ch) =>
					ch.isTextBased() &&
					(ch as TextChannel).permissionsFor(queue.guild.members.me!)?.has("SendMessages")
			) as TextChannel | undefined;

			if (defaultChannel) {
				queue.metadata = { channel: defaultChannel };
				console.log(`[${timestamp}] INFO: Default metadata set with channel: ${defaultChannel.id}`);
			} else {
				console.warn(`[${timestamp}] WARN: Keine Metadata oder kein Channel in der Queue vorhanden, und kein Default gefunden.`);
				return;
			}
		}
