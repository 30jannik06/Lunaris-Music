...
export async function execute(interaction: ChatInputCommandInteraction) {
	...
	if (!queue) {
		return interaction.reply({ content: "This server does not have an active player session", ephemeral: true });
	}
	...
	const embed = new EmbedBuilder()
		...
		.setDescription(`Loop mode set to **${repeatModeNames[loopMode as QueueRepeatMode]}**`)
		.setColor("Blue")
		.setTimestamp();
