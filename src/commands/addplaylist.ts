...
export const data = new SlashCommandBuilder()
	.setName("addplaylist")
	.setDescription("Add a Playlist from a given URL or query to the queue")
	.setDescriptionLocalizations({
		de: "Hinzufügen einer Wiedergabeliste von einer bestimmten URL oder Abfrage zur Warteschlange",
		"en-US": "Add a Playlist from a given URL or query to the queue"
	})
	.addStringOption((option) =>
		option
			...
			.setDescriptionLocalizations({
				de: "Die Playlist-URL oder Suchanfrage",
				"en-US": "The playlist URL or search query",
			})
			.setRequired(true),
	);
...
export async function execute(interaction: ChatInputCommandInteraction) {
	...
	try {
		...
		if (updatedQueue) {
			...
			const lastTrack: Track | undefined =
				tracksArray.length > 0
					? tracksArray[tracksArray.length - 1]
					: undefined;
			const addedTrack: Track | undefined =
				(lastTrack || updatedQueue.currentTrack) ?? undefined;
			if (addedTrack) {
		...
		return interaction.editReply(
			`Song wurde der Queue hinzugefügt: ${safeQuery}`,
		);
	} catch (error: any) {
