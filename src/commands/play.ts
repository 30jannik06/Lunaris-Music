...
import { useQueue, useMainPlayer, Track } from "discord-player";
...
export async function execute(interaction: ChatInputCommandInteraction) {
	...
	if (existingQueue) {
		...
		const isAlreadyQueued = existingQueue.tracks.some((track: Track) => track.url === safeQuery);
		const isCurrentlyPlaying = existingQueue.currentTrack && existingQueue.currentTrack.url === safeQuery;
		if (isAlreadyQueued || isCurrentlyPlaying) {
			return interaction.editReply("Der Song ist bereits in der Queue oder wird gerade abgespielt.");
		}
	}
	...
	try {
		...
		if (track) {
			...
			return interaction.editReply(`Füge den Song zur Queue hinzu: ${safeQuery}`);
		}
	} catch (error: any) {
		...
		return interaction.editReply("Beim Abspielen des Songs ist ein Fehler aufgetreten.");
	}
}
